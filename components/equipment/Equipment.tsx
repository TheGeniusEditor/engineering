      ...
      <div style={{ borderRadius: 12, background: "#0f172a", color: "white", padding: "20px 24px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, position: "relative" }}>
          ...
          {activeRole.canManageEquipment && (
            <button onClick={() => router.push("/equipment/new")} style={{ display: "flex", alignItems: "center", gap: 7, padding: "9px 16px", borderRadius: 8, background: "white", color: "#0f172a", border: "none", fontSize: 13, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>
      ...
      <div style={{ borderRadius: 12, border: "1px solid #e2e8f0", background: "white", overflow: "hidden" }}>
        ...
        <div style={{ padding: "14px 18px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 8, background: "#0f172a", color: "white", border: "none", fontSize: 12, fontWeight: 700, cursor: "pointer" }}
          >
        ...
        {/* Inline form */}
        {showNonQRForm && (
          <div style={{ padding: "18px 20px", borderBottom: "1px solid #f1f5f9", background: "#fafafa" }}>
            <div style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: "#64748b", marginBottom: 14 }}>New Non-QR Item</div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <label className="label">Item Name <span className="text-red-500">*</span></label>
                <input className="input" placeholder="e.g. Door Handle – Lever Type" value={nonQRForm.name}
                  onChange={e => { setNonQRForm(f => ({ ...f, name: e.target.value })); setNonQRErrors(err => ({ ...err, name: "" })); }} />
                {nonQRErrors.name && <p style={{ fontSize: 11, color: "#dc2626", marginTop: 4 }}>{nonQRErrors.name}</p>}
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                ...
                <input className="input" placeholder="e.g. Tower A – Guest Floors 10–20" value={nonQRForm.location}
                  onChange={e => { setNonQRForm(f => ({ ...f, location: e.target.value })); setNonQRErrors(err => ({ ...err, location: "" })); }} />
                {nonQRErrors.location && <p style={{ fontSize: 11, color: "#dc2626", marginTop: 4 }}>{nonQRErrors.location}</p>}
              </div>
              <div>
                <label className="label">Quantity <span className="text-red-500">*</span></label>
                <input type="number" min={1} className="input" value={nonQRForm.quantity} onChange={e => setNonQRForm(f => ({ ...f, quantity: Number(e.target.value) }))} />
              </div>
              <div>
