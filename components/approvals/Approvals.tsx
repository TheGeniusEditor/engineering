...
export default function Approvals() {
  ...
  const shownApprovals  = filter === "pending" ? approvalsSeed.filter(a => a.status !== "Approved") : approvalsSeed;
    ...
    <div className="space-y-5">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
        {[
          ...
          { label: "Total This Week", value: approvalsSeed.length + pendingRequests.length,           icon: ShieldCheck,  warn: false },
        ].map((kpi, i) => {
          const Icon = kpi.icon;
          return (
            <div key={i} style={{
